...
export default defineConfig({
  ...
  dev: {
    ...(!isDevServer ? {assetPrefix: '/bundles/pimcorequill/studio/build/' + buildId} : {}),
    client: {
      host: 'localhost',
      port: 3032,
      protocol: 'ws'
  ...
  output: {
    manifest: true,
    assetPrefix: '/bundles/pimcorequill/studio/build/' + buildId,
    distPath: {
      root: buildPath
    },
  },
  tools: {
    bundlerChain: (chain, { env }) => {
      chain.output.uniqueName('pimcore_quill_bundle');
    },
  },
  plugins: [
